...
2. **`Speech_Detection-main/` (Speech Modality)**:
   A fully decoupled toolkit for voice interaction. This includes Push-to-Talk logic, speech-to-text translation (via models like Whisper/Vosk), intent classification (determining *what* the user meant), and dialogue management (asking for clarifications if needed). It publishes standard `SpeechEvent` data.
...
2. **Intent Recognition:** When you hold the microphone key and speak, the system parses the sentence using an ASR engine and Regex patterns to determine your intent (e.g., `ADD_TO_CART`, `SCROLL_DOWN`).
