...
@pytest.fixture
def parser():
    """IntentParser loaded with Shopping vocabulary (the reference app)."""
    return IntentParser(Config(), custom_patterns=SHOPPING_INTENTS)
...
class TestCustomPatterns:
    ...
    def test_custom_pattern_does_not_break_builtins(self, parser):
        """Adding a custom pattern does not interfere with shopping intents."""
        parser.register_pattern(IntentPattern(
            intent="CUSTOM_CMD",
            target_required=False,
            patterns=[r"\bcustom\b"],
        ))
        # Shopping intent still works
        event = parser.parse("add this to cart")
        assert event.payload["intent"] == "ADD_TO_CART"
