...
# ── Tuning constants ──────────────────────────────────────────────────────────
_COLLECT_S        = 1.00    # seconds to collect stable samples per calibration point
_MIN_COLLECT      = 12      # minimum valid frames required; ~30 fps × 1.0 s ≈ 30 frames
_QUALITY_THRESH   = 100.0   # max acceptable RMSE in px before rejecting calibration
_CALIB_STD_WARN   = 0.06    # feature-space std threshold above which a point is flagged unstable
...
def eye_relative_gaze_from_landmarks(lms) -> tuple[float, float] | None:
    ...
    for iris_idx, ca_idx, cb_idx, top_idx, bot_idx in (
        ...
        try:
            ...
            # 0.12 catches partial blinks that 0.08 missed, without rejecting
            # normally-narrow eyes on most hardware.
            if eye_h / eye_w < 0.12:
                continue
...
class GazeFeatureSmoother:
    ...
    @property
    def smooth_value(self) -> tuple[float, float] | None:
        """Last smoothed feature value (for debug overlay)."""
        return self._prev
...
class GazeCalibrator:
    ...
    def __init__(self, win_w: int = 1280, win_h: int = 720) -> None:
        ...
        # RMSE of last fit_and_validate() call — exposed for debug overlay
...
class CalibSession5:
    ...
    * After all 5 points, ``fit_and_validate`` checks RMSE.  If the error
    ...
    def __init__(self, calibrator: "GazeCalibrator5") -> None:
        ...
        # Count of points flagged as unstable during collection
        self._unstable_points: int = 0
    ...
    def tick(self, gaze_feat: tuple[float, float] | None) -> bool:
        ...
        # Warn when samples are too scattered (gaze was unstable during collection)
        std_x = float(np.std(xs))
        std_y = float(np.std(ys))
        if std_x > _CALIB_STD_WARN or std_y > _CALIB_STD_WARN:
            logger.warning(
                "[CAL5] Point %d: high sample variance std=(%.3f,%.3f) — "
                "keep eyes still on target.", self._idx, std_x, std_y)
            self._unstable_points += 1

        # Add a repeated copy so lstsq sees this point with reasonable weight
        for _ in range(12):
            self._cal.add_sample(self._idx, med_feat)

        logger.info("[CAL5] Point %d stored: (%.4f, %.4f)  from %d frames  std=(%.3f,%.3f)",
                    self._idx, *med_feat, n_valid, std_x, std_y)
    ...
    def _finish(self) -> bool:
        ...
        rmse = getattr(self._cal, 'last_rmse', None)
        rmse_str = f"  RMSE {rmse:.0f} px" if rmse is not None else ""
        unstable_str = (f"  ({self._unstable_points} unstable point(s) — recalibrate for best accuracy)"
                        if self._unstable_points > 0 else "")
        if result == 'ok':
            self.status_msg = f"Calibration complete!{rmse_str}{unstable_str}"
        elif result == 'poor':
            self.status_msg = (
                f"Calibration poor{rmse_str} — keep head still and retry with [U]."
            )
        else:
            self.status_msg = "Calibration FAILED – press U to retry."
        self.active = False
        logger.info("[CAL5] Finished — result: %s  rmse: %s  unstable_pts: %d",
                    result, rmse_str.strip(), self._unstable_points)
    ...
    def draw_overlay(self, canvas: "np.ndarray",
        ...
        _cv2.putText(canvas, "Keep head still  |  Move eyes not head  |  Use good lighting",
                     (W // 2 - 240, 84),
                     _cv2.FONT_HERSHEY_SIMPLEX, 0.44, (160, 220, 160), 1, _cv2.LINE_AA)
        _cv2.putText(canvas, "Look directly at the dot  |  Recalibrate with [U] if gaze drifts",
                     (W // 2 - 240, 104),
                     _cv2.FONT_HERSHEY_SIMPLEX, 0.40, (130, 180, 130), 1, _cv2.LINE_AA)
