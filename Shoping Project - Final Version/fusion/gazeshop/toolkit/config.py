...
class Config:
    ...
    """Weights for the three-component heuristic confidence score.
    Must sum to 1.0.  See IntentParser._compute_confidence() for formula."""

    FUSION_CONFIDENCE_BIAS: float = 1.0
    """Additive bias applied to ASR confidence when computing the fused
    confidence score in FusionEngine.  The formula is:

        fused_confidence = (asr_confidence + FUSION_CONFIDENCE_BIAS) / 2

    The default value of 1.0 gives equal weight to the ASR confidence
    and a perfect-gaze prior, matching the original hardcoded behaviour.
    Lower values (e.g. 0.8) reduce the gaze-lock bonus; values above 1.0
    are clamped naturally by the ``round()`` call in FusionEngine."""
