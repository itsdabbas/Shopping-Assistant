...
# ── Minimal generic Vosk grammar fallback (dialog-core phrases only).
# Applications should supply their own vocabulary via the ``vosk_vocab``
# constructor parameter so the toolkit remains domain-agnostic.
_VOSK_FALLBACK_VOCAB: list[str] = [
    "yes",
    "no",
    "cancel",
    "help",
...
class SpeechAdapter(ModalityAdapter):
    ...
    vosk_vocab:
        Application-specific list of phrases to pass to the Vosk grammar
        recogniser.  ``"[unk]"`` is appended automatically if not already
        present.  When ``None`` the adapter falls back to a minimal
        dialog-core vocabulary so the toolkit stays domain-agnostic.
        Pass the full domain vocabulary from your app layer (e.g.
        ``SHOPPING_VOSK_VOCAB`` from ``apps.shopping.intents``).
    """
    ...
    def __init__(
        ...
        vosk_vocab: list[str] | None = None,
    ) -> None:
        super().__init__(event_bus)
        self.config = config or Config()
        self.parser = intent_parser or IntentParser(self.config)

        # ── Vosk grammar JSON (built once; recreated per recogniser session) ──
        vocab = list(vosk_vocab) if vosk_vocab is not None else list(_VOSK_FALLBACK_VOCAB)
        if "[unk]" not in vocab:
            vocab.append("[unk]")
        self._vosk_vocab_json: str = json.dumps(vocab)
    ...
    def _init_vosk(self) -> None:
        try:
            ...
            self._asr_engine = KaldiRecognizer(self._vosk_model, self.config.SAMPLE_RATE, self._vosk_vocab_json)
