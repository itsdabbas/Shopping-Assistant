...
class GazeAdapterStub(ModalityAdapter):
    ...
    def simulate_dwell(
        ...
        """Simulate a user dwelling on *target_id* until a LOCK fires.

        Blocks the calling thread for *duration_s* seconds (simulating the
        dwell period), then emits a LOCK ``GazeEvent`` identical to what
        :meth:`simulate_lock` would produce.

        Parameters
        ----------
        target_id:
            ID of the item the simulated user is looking at.
        duration_s:
            Dwell duration in seconds.  If ``None``, uses
            ``self.dwell_to_lock_s`` (set at construction time).
        confidence:
            Simulated gaze confidence for the resulting LOCK event.

        Returns
        -------
        GazeEvent
            The emitted LOCK event (same object as :meth:`simulate_lock`).
        """
        wait = duration_s if duration_s is not None else self.dwell_to_lock_s
        _time.sleep(wait)
        return self.simulate_lock(target_id, confidence=confidence)
