...
INTENT_SYNONYMS: dict[str, tuple[str, list[str]]] = {
    "add to cart":    ("ADD_CART",     ["add cart", "put in cart", "add this", "cart add", "add", "cart", "card", "two cart", "two card", "to cart", "to card"]),
    "show details":   ("SHOW_DETAIL",  ["details", "show detail", "more info", "tell me more"]),
    "find similar":   ("FIND_SIMILAR", ["similar items", "find like", "show similar"]),
    "compare items":  ("COMPARE",      ["compare", "comparison", "compare these"]),
    "go back":        ("NAV_BACK",     ["back", "previous", "go previous"]),
