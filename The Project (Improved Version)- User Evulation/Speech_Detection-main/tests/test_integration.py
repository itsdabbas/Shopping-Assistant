...
from apps.shopping.intents import SHOPPING_INTENTS
...
def speech(bus):
    ...
    return SpeechAdapter(event_bus=bus, config=cfg, intent_parser=parser)
...
class TestSimulatedFusion:
    ...
    def test_ambiguous_gaze_triggers_disambiguation(self, bus, speech, gaze):
        ...
        # GazeAdapterStub formats candidates as [{"id":..., "pos":...}, ...]
        candidate_ids = [c["id"] for c in candidates]
        assert "item_3" in candidate_ids
        assert "item_4" in candidate_ids
        ...
        # Resolve: "left" → first candidate (candidates are dicts with "id"/"pos")
        resolved_id = candidates[0]["id"]
        assert resolved_id == "item_3"
